...
from ce_scheme3.reference_model import ContrastConfig, FrameResult, compute_histogram
...
class FloatDiscreteSceneGainModel:
    ...
    def _detect_pattern_bypass(self, plane: np.ndarray) -> bool:
        ...
        return self._pattern_histogram_candidate(plane)

    def _pattern_histogram_features(self, plane: np.ndarray) -> dict[str, int]:
        ...
        hist = np.bincount((values.reshape(-1) >> shift), minlength=self.cfg.pattern_hist_bin_count)
        active_mask = hist > 0
    ...
    def _pattern_histogram_candidate(self, plane: np.ndarray) -> bool:
        features = self._pattern_histogram_features(plane)
        ...
        return dense_gradient_candidate or sparse_pattern_candidate or comb_candidate
