...
def test_histogram_like_ramp_but_spatially_shuffled_does_not_bypass():
    ...
    assert result.bypass_flag is False
    assert not np.array_equal(enhanced, shuffled)


def test_pattern_bypass_uses_histogram_candidate_plus_streaming_confirmation():
    ...
    assert model._pattern_streaming_confirmation(ramp) is True
    assert model._detect_pattern_bypass(ramp) is True

    assert model._pattern_histogram_candidate(shuffled) is True
    assert model._pattern_streaming_confirmation(shuffled) is False
    assert model._detect_pattern_bypass(shuffled) is False
