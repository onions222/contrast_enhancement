...
% gain_max_q8: U4.8，寄存器，可调，1.0 -> 256。
% 含义：
%   - 主增强段允许的最大斜率
% 作用：
%   - 防止输入动态范围较窄时，主段斜率过大
%   - 过大会把皮肤纹理、噪声、细小亮暗起伏一起放大
%   - 当前默认值偏保守，目的是优先压住人脸 close-up 的劣化
cfg.gain_max_q8 = uint16(256);

% toe_margin: U8.0，寄存器，可调，默认 24。
% 含义：
%   - 输出端暗部预留的保护余量
%   - 即 y_low
% 作用：
...
cfg.toe_margin = uint16(24);
% shoulder_margin: U8.0，寄存器，可调，默认 24。
% 含义：
%   - 输出端高光预留的保护余量
%   - 即 255 - y_high
% 作用：
